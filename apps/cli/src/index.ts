...
try {
  switch(command){
    ...
    case "memory:supersede":await supersede(arg1,arg2);break; case "memory:expire":await expire();break;
    default: console.error("Usage: ai-os <doctor|db:migrate|registry:validate|registry:sync|session:validate|session:import|memory:import|memory:list|memory:invalidate|memory:supersede|memory:expire> [args]"); process.exitCode=1;
  }
