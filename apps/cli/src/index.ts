...
import { createDefaultAdapterRegistry } from "@ai-os/provider-adapters";
...
async function importProviderExport(pathArg?: string, projectId?: string, providerHint?: string): Promise<void> {
  if(!pathArg||!projectId) throw new Error("provider:import requires path and project-id");
  const path=resolve(pathArg); const registry=await createDefaultAdapterRegistry();
  const source=providerHint?{path,projectId,providerHint}:{path,projectId};
  const adapter=await registry.resolve(source); const archives=await adapter.parse(source);
  const fileHash=createHash("sha256").update(await readFile(path)).digest("hex");
  const imported=await withDatabase(db=>archives.map(archive=>{
    upsertSession(db,{id:archive.id,projectId:archive.projectId,provider:archive.provider,model:archive.model,startedAt:archive.startedAt,endedAt:archive.endedAt,archivePath:`${path}#${archive.id}`,contentHash:fileHash});
    return {id:archive.id,messages:replaceSessionMessages(db,archive)};
  }));
  console.log(JSON.stringify({adapter:adapter.id,path,count:imported.length,imported},null,2));
}
...
try {
  switch(command){
    ...
    case "provider:import":await importProviderExport(args[0],args[1],args[2]);break;
    ...
    default: console.error("Usage: ai-os <doctor|db:migrate|registry:validate|registry:sync|session:validate|session:import|archive:import|archive:import-dir|archive:search|provider:import|memory:import|memory:list|memory:invalidate|memory:supersede|memory:expire> [args]"); process.exitCode=1;
  }
