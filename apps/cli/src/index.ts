...
async function validateImportSources(pathArg?: string): Promise<void> {
...
async function syncConfiguredImportSources(pathArg?: string, sourceId?: string, forceArg?: string): Promise<void> {
  const path=resolve(pathArg??"config/import-sources.yaml");
  const registry=await loadImportSourceRegistry(path);
  const force=sourceId==="--force"||forceArg==="--force";
  const selectedSourceId=sourceId&&sourceId!=="--force"?sourceId:undefined;
  const results=await withDatabase(db=>syncImportSources(db,registry,{...(selectedSourceId?{sourceId:selectedSourceId}:{}),...(force?{force:true}:{})}));
  const summary=results.reduce((counts,result)=>({...counts,[result.status]:(counts[result.status]??0)+1}),{} as Record<string,number>);
  console.log(JSON.stringify({path,summary,results},null,2));
