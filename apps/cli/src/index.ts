...
try {
  switch(command){
    ...
    case "archive:import":await importArchive(args[0],args[1],args[2],args[3]);break; case "archive:search":await searchArchive(args[0],args[1]);break;
    ...
    default: console.error("Usage: ai-os <doctor|db:migrate|registry:validate|registry:sync|session:validate|session:import|archive:import|archive:search|memory:import|memory:list|memory:invalidate|memory:supersede|memory:expire> [args]"); process.exitCode=1;
  }
