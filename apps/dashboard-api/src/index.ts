...
body{font-family:system-ui,sans-serif;max-width:1100px;margin:0 auto;padding:24px;background:#f6f7f9;color:#15171a}h1{margin-bottom:4px}.muted{color:#666}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:20px 0}.card{background:white;border:1px solid #ddd;border-radius:10px;padding:16px}.controls,.tabs{display:flex;gap:8px;flex-wrap:wrap;margin:16px 0}input,select,button{padding:10px;border:1px solid #bbb;border-radius:8px;background:white}input{min-width:240px}button{cursor:pointer}.active{font-weight:700;border-color:#175cd3}.result{white-space:pre-wrap;font-family:ui-monospace,monospace;font-size:13px}.row{border-top:1px solid #eee;padding:10px 0}.pager{display:flex;gap:8px;align-items:center;margin-top:12px}.badge{display:inline-block;padding:2px 7px;border:1px solid #bbb;border-radius:99px;font-size:12px}a{color:#175cd3;text-decoration:none}a:hover{text-decoration:underline}@media(max-width:700px){.grid{grid-template-columns:1fr}input{min-width:100%}}
</style></head><body>
<h1>AI OS Dashboard</h1><div class="muted">Local read-only view of projects, sessions, indexed messages and provider imports.</div>
<div id="stats" class="grid"></div>
<div class="tabs"><button id="sessionsTab" class="active">Sessions</button><button id="importsTab">Import history</button></div>
<div class="card"><div id="sessionControls" class="controls"><select id="project"></select><input id="query" placeholder="Search session messages"><button id="search">Search</button><button id="sessions">Recent sessions</button></div><div id="importControls" class="controls" hidden><select id="importProject"></select><select id="provider"><option value="">All providers</option><option>chatgpt</option><option>codex</option><option>opencodex</option><option>gemini</option><option>claude</option><option>ollama</option></select><select id="status"><option value="">All statuses</option><option>running</option><option>succeeded</option><option>failed</option><option>skipped</option></select><button id="refreshImports">Refresh</button></div><div id="results"></div><div class="pager"><button id="prev">Previous</button><span id="page"></span><button id="next">Next</button></div></div>
<script>
...
function showImports(){page=1;sessionsTab.classList.remove('active');importsTab.classList.add('active');sessionControls.hidden=true;importControls.hidden=false;loadImports()}
function showSessions(){page=1;importsTab.classList.remove('active');sessionsTab.classList.add('active');importControls.hidden=true;sessionControls.hidden=false;loadSessions()}
sessionsTab.onclick=showSessions;importsTab.onclick=showImports;search.onclick=()=>{page=1;runSearch()};sessions.onclick=()=>{page=1;loadSessions()};project.onchange=()=>{page=1;mode==='search'?runSearch():loadSessions()};refreshImports.onclick=()=>{page=1;loadImports()};importProject.onchange=refreshImports.onclick;provider.onchange=refreshImports.onclick;status.onchange=refreshImports.onclick;prev.onclick=()=>{if(page>1){page--;mode==='search'?runSearch():mode==='imports'?loadImports():loadSessions()}};next.onclick=()=>{page++;mode==='search'?runSearch():mode==='imports'?loadImports():loadSessions()};query.onkeydown=e=>{if(e.key==='Enter'){page=1;runSearch()}};init().catch(e=>results.textContent=e.message);
</script></body></html>`;
